...
- **FORBIDDEN previews include reasons** — `audit --dry-run` (CLI) and `server_fix` dry-run (MCP) responses now include a `forbiddenReason` for each FORBIDDEN-tier fix (e.g. "requires reboot", "disrupts active SSH sessions", "can lock out remote operators"). The 52 FORBIDDEN definition sites (SSH:2, Firewall:17, Docker:33) carry this metadata; parser expansion produces a 71-check runtime inventory (SSH:22, Firewall:17, Docker:32).
