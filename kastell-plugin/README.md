...
The Kastell plugin bundles first-party MCP tools, 4 skills, 1 agent, and 5 hooks that give Claude Code
full control over your self-hosted server infrastructure. Use it to provision cloud servers,
run security audits across all categories, apply production hardening, manage backups,
...
**`/agent:kastell-auditor`** — Parallel audit analyzer that groups all audit categories into
...
All first-party MCP tools are available in Claude Code once the plugin is installed. The MCP server starts
...
| server_audit | Run the full security audit across all categories |
...
| server_lock | Apply the production hardening sequence |
